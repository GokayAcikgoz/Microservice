...
namespace Microservice.Catalog.Api.Features.Courses.Create
{
    public class CreateCourseCommandHandler(AppDbContext context, IMapper mapper) : IRequestHandler<CreateCourseCommand, ServiceResult<Guid>>
    {
        public async Task<ServiceResult<Guid>> Handle(CreateCourseCommand request, CancellationToken cancellationToken)
        {
            ...
            if (!hasCategory)
            {
                return ServiceResult<Guid>.Error("Category not found", $"The Category with id{request.CategoryId} wat not found", HttpStatusCode.NotFound);
            }   
            ...
            {
                Duration = 10,
                EducationFullName = "Ahmet Yılmaz",
                Rating = 0
            };
